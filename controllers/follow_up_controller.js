...
const postFollowUp = async (req, res) => {
  try {
    const {baby_id, follow_up_date, motorMilestones, feedingMilestones, communicationMilestones, sensoryMilestones, score,  notes} = req.body;
    const query = `INSERT INTO follow_up (baby_id, follow_up_date, motorMilestones, feedingMilestones, communicationMilestones, sensoryMilestones, score,  notes) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`;
    const result = await pool.query(query, [baby_id, follow_up_date, motorMilestones, feedingMilestones, communicationMilestones, sensoryMilestones, score,  notes]);
    res.status(200).json(result.rows[0]);
  } catch (e) {
    console.error(e);
    res.status(500).json({error: `server error: ${e}`});
